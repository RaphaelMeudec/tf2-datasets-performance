...
@click.option(
    ...
    type=bool,
    default=False,
    help="Whether or not to enable float16 execution",
)
def run_analysis(epochs, steps_per_epoch, batch_size, dataset_path, enable_float16):
    logs_dir = Path("logs") / str(datetime.timestamp(datetime.now()))
    logger.add(
        logs_dir
        / f"epochs_{epochs}_steps_{steps_per_epoch}_batch_{batch_size}_float16_{enable_float16}.log"
    )
    ...
    dtype = tf.float16 if enable_float16 else tf.float32
    model.fit(
